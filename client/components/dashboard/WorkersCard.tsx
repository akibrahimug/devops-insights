...
/**
 * Component: WorkersCard
 * I display worker process utilization and related key insights for a region.
 * I summarize busy/idle/waiting counts, show a utilization bar, and reveal
 * recently blocked keys and top keys with optional masking and copy support.
 */
...
export function WorkersCard({
   ...
   * @param k - The original key string
   * @param reveal - When true, I return the original key
   */
      ...
      <CardContent>
        <div className="space-y-4">
          {workers.map(([name, data], index) => {
            ...
            // Normalize blocked and top keys as small arrays ready for rendering.
            ...
            // Determine if there is any content worth showing for this worker.
            const hasData =
              (totalWorkers || 0) > 0 ||
              (idle || 0) > 0 ||
              (waiting || 0) > 0 ||
            ...
            const panelClass = [
              "border rounded-lg p-4 bg-white/50 dark:bg-gray-900/40 backdrop-blur-sm transition-colors hover:bg-gray-50 dark:hover:bg-gray-900/30",
              isHistoryMode && !hasData
              ...
              <div key={index} className={panelClass}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <GearSix className="h-8 w-8" style={{ color }} />
                    <div>
                        ...
                        <AccordionContent>
                          <div className="flex items-center justify-end mb-1">
                            <button
                              ...
                              aria-label={
                                showTopKeys[name] ? "Hide keys" : "Show keys"
                              }
                            >
                              {showTopKeys[name] ? (
                                <EyeSlash className="h-4 w-4" />
